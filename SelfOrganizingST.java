...
public class SelfOrganizingST<Key, Value> {
    ...
    /**
     * Find the node that occurs before the one associated with the current key
     * @param key the key of current node
    ...
    public Node find(Key key) {
        ...
        CompCount++;
        if (curr.next == sentinel) return sentinel; // not found
